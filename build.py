...
support_targets = ['myfuck', 'myfuckinit', 'myfuckboot', 'myfuckpolicy', 'resetprop', 'busybox', 'test']
default_targets = ['myfuck', 'myfuckinit', 'myfuckboot', 'busybox']
    ...
    if is_windows:
        ...
        if not op.exists(elf_cleaner):
            execv(['g++', '-std=c++11', 'tools/termux-elf-cleaner/termux-elf-cleaner.cpp',
                   '-o', elf_cleaner])
    args = [elf_cleaner]
    args.extend(op.join('native', 'out', arch, 'myfuck')
                for arch in archs + arch64)
    execv(args)
