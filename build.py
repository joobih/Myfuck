...
default_targets = ['myfuck', 'myfuckinit', 'myfuckboot', 'myfuckpolicy', 'busybox']
support_targets = default_targets + ['resetprop', 'test']
rust_targets = ['myfuck', 'myfuckinit', 'myfuckboot', 'myfuckpolicy']

sdk_path = os.environ['ANDROID_SDK_ROOT']
ndk_root = op.join(sdk_path, 'ndk')
ndk_path = op.join(ndk_root, 'myfuck')
...
def clean_elf():
    ...
    args.extend(op.join('native', 'out', arch, bin)
                for arch in archs for bin in ['myfuck', 'myfuckpolicy'])
    execv(args)
...
def dump_bin_header(args):
    ...
    if not op.exists(stub):
        error('Build stub APK before building "myfuckinit"')
...
def dump_flag_header():
    flag_txt = textwrap.dedent('''\
        ...
        ''')
    flag_txt += f'#define MYFUCK_VERSION      "{config["version"]}"\n'
    flag_txt += f'#define MYFUCK_VER_CODE     {config["versionCode"]}\n'
    flag_txt += f'#define MYFUCK_DEBUG        {0 if args.release else 1}\n'
...
def build_binary(args):
    ...
    if 'myfuckpolicy' in args.target:
        flag += ' B_POLICY=1'

    if 'test' in args.target:
        flag += ' B_TEST=1'

    if 'myfuckinit' in args.target:
        flag += ' B_PRELOAD=1'

    if 'resetprop' in args.target:
        flag += ' B_PROP=1'

    if 'myfuckboot' in args.target:
        flag += ' B_BOOT=1'
    ...
    # myfuckinit and busybox has to be built separately
...
def build_app(args):
    header('* Building the Myfuck app')
    build_apk(args, 'app')
...
def setup_avd(args):
    ...
    proc = execv([adb_path, 'push', f'native/out/{abi}/busybox', 'out/app-debug.apk',
           'scripts/avd_myfuck.sh', '/data/local/tmp'])
    if proc.returncode != 0:
        error('adb push failed!')

    proc = execv([adb_path, 'shell', 'sh', '/data/local/tmp/avd_myfuck.sh'])
    if proc.returncode != 0:
        error('avd_myfuck.sh failed!')
...
parser = argparse.ArgumentParser(description='Myfuck build script')
...
app_parser = subparsers.add_parser('app', help='build the Myfuck app')
...
ndk_parser = subparsers.add_parser('ndk', help='setup Myfuck NDK')
