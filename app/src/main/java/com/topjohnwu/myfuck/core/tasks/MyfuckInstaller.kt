...
abstract class MyfuckInstallImpl protected constructor(
    ...
    private fun processTar(input: InputStream, output: OutputStream): OutputStream {
        ...
        if (Config.recovery && recovery.exists() && boot.exists()) {
            ...
            arrayOf(
                "cd $installDir",
                "chmod -R 755 .",
                "./myfuckboot unpack boot.img",
                "./myfuckboot repack boot.img",
    ...
    private fun patchBoot(): Boolean {
        ...
        val job = shell.newJob().add("./myfuckboot cleanup", "cd /")
...
abstract class MyfuckInstaller(
    console: MutableList<String>,
    logs: MutableList<String>
) : MyfuckInstallImpl(console, logs) {
    ...
    class FixEnv(private val callback: () -> Unit) : MyfuckInstallImpl() {
        override suspend fun operations() = fixEnv()
