...
env_check() {
  ...
  if [ "$2" -ge 25000 ]; then
    [ -f "$MYFUCKBIN/myfuckpolicy" ] || return 1
  fi
  grep -xqF "MYFUCK_VER='$1'" "$MYFUCKBIN/util_functions.sh" || return 1
  grep -xqF "MYFUCK_VER_CODE=$2" "$MYFUCKBIN/util_functions.sh" || return 1
  return 0
}
...
restore_imgs() {
  [ -z $SHA1 ] && return 1
  local BACKUPDIR=/data/myfuck_backup_$SHA1
...
versionCode=1
author=Myfuck
description=Myfuck app built-in systemless hosts module
EOF
  myfuck --clone /system/etc/hosts hosts/system/etc/hosts
  touch hosts/update
  cd /
}
