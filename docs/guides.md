...
#### sepolicy.rule
If your module requires some additional sepolicy patches, please add those rules into this file. The module installer script and Myfuck's daemon will make sure this file is copied to somewhere `myfuckinit` can read pre-init to ensure these rules are injected properly.

Each line in this file will be treated as a policy statement. For more details about how a policy statement is formatted, please check [myfuckpolicy](tools.md#myfuckpolicy)'s documentation.

#### The `system` folder
All files you want Myfuck to replace/inject for you should be placed in this folder. Please read through the [Magic Mount](details.md#magic-mount) section to understand how Myfuck mount your files.
...
- When your module is downloaded with the Myfuck app, `update-binary` will be **forcefully** replaced with the latest [`module_installer.sh`](https://github.com/topjohnwu/Myfuck/blob/master/scripts/module_installer.sh) to ensure all installer uses up-to-date scripts. **DO NOT** try to add any custom logic in `update-binary` as it is pointless.
...
You can submit a module to **Myfuck-Module-Repo** so users can download your module directly in the Myfuck app.
...
- Create a request for submission via this link: [submission](https://github.com/Myfuck-Modules-Repo/submission)
...
These scripts will run in Myfuck's BusyBox `ash` shell with "Standalone Mode" enabled.
...
│   │   └── myscript.sh    <--- to Myfuck's tmpfs directory
