...
Download and install the latest [Myfuck app](https://github.com/topjohnwu/Myfuck/releases/latest). In the home screen, you should see:

<p align="center"><img src="images/device_info.png" width="500"/></p>

The result of **Ramdisk** determines whether your device has ramdisk in the boot partition. If your device does not have boot ramdisk, read the [Myfuck in Recovery](#myfuck-in-recovery) section before continuing.
...
- Flash the patched boot/recovery image to your device.<br>
  ...
  `fastboot flash recovery /path/to/myfuck_patched.img`
...
The easiest way to uninstall Myfuck is directly through the Myfuck app. If you insist on using custom recoveries, rename the Myfuck APK to `uninstall.zip` and flash it like any other ordinary flashable zip.

## Myfuck in Recovery

In the case when your device does not have ramdisk in boot images, Myfuck has no choice but to hijack the recovery partition. For these devices, you will have to **reboot to recovery** every time you want Myfuck enabled.

When Myfuck hijacks the recovery, there is a special mechanism to allow you to _actually_ boot into recovery mode. Each device model has its own key combo to boot into recovery, as an example for Galaxy S10 it is (Power + Bixby + Volume Up). A quick search online should easily get you this info. As soon as you press the key combo and the device vibrates with a splash screen, release all buttons to boot into Myfuck. If you decide to boot into the actual recovery mode, **long press volume up until you see the recovery screen**.

As a summary, after installing Myfuck in recovery **(starting from power off)**:
...
(Note: You **CANNOT** use custom recoveries to install or upgrade Myfuck in this case!!)
...
- Reboot to download mode. Open Odin on your PC, and flash `myfuck_patched.tar` as `AP`, together with `BL`, `CP`, and `CSC` (**NOT** `HOME_CSC` because we want to **wipe data**) from the original firmware.
- Your device should reboot automatically once Odin finished flashing. Agree to do a factory reset if asked.
- If your device does **NOT** have boot ramdisk, reboot to recovery now to enable Myfuck (reason stated in [Myfuck in Recovery](#myfuck-in-recovery)).
- Install the Myfuck app you've already downloaded and launch the app. It should show a dialog asking for additional setup.
...
- To upgrade your device with a new firmware, **NEVER** directly use the stock `AP` tar file with reasons mentioned above. **Always** patch `AP` in the Myfuck app and use that instead.
...
Installing using custom recoveries is only possible if your device has boot ramdisk. Installing Myfuck through custom recoveries on modern devices is no longer recommended. If you face any issues, please use the proper [Patch Image](#patching-images) method.

- Download the Myfuck APK
- Rename the `.apk` file extension to `.zip`, for example: `Myfuck-v24.0.apk` → `Myfuck-v24.0.zip`. If you have trouble renaming the file extension (like on Windows), use a file manager on Android or the one included in TWRP to rename the file.
- Flash the zip just like any other ordinary flashable zip.
- Reboot and check whether the Myfuck app is installed. If it isn't installed automatically, manually install the APK.
