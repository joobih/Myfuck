...
fun Project.setupCommon() {
    androidBase {
        ...
        ndkPath = "$sdkDirectory/ndk/myfuck"
...
fun Project.setupApp() {
    ...
    val syncLibs = tasks.register("syncLibs", Sync::class.java) {
        ...
        into("armeabi-v7a") {
            from(rootProject.file("native/out/armeabi-v7a")) {
                include("busybox", "myfuckboot", "myfuckinit", "myfuckpolicy", "myfuck")
                rename { if (it == "myfuck") "libmyfuck32.so" else "lib$it.so" }
            }
        }
        into("x86") {
        ...
        into("arm64-v8a") {
            from(rootProject.file("native/out/arm64-v8a")) {
                include("busybox", "myfuckboot", "myfuckinit", "myfuckpolicy", "myfuck")
                rename { if (it == "myfuck") "libmyfuck64.so" else "lib$it.so" }
            }
        }
        into("x86_64") {
    ...
    android.applicationVariants.all {
        ...
        val outSrc = File(outSrcDir, "com/topjohnwu/myfuck/signing/KeyData.java")
