...
extern "C" {
    ...
    fn myfuck_log_write(prio: i32, msg: *const u8, len: i32);
...
pub fn android_logging() {
    fn android_log_fmt(level: LogLevel, args: Arguments) {
        ...
        unsafe {
            __android_log_write(level_to_prio(level), b"Myfuck\0".as_ptr(), buf.as_ptr());
        }
    }
    fn android_log_write(level: LogLevel, msg: &[u8]) {
        unsafe {
            __android_log_write(level_to_prio(level), b"Myfuck\0".as_ptr(), msg.as_ptr());
        }
    }
...
pub fn myfuck_logging() {
    fn myfuck_fmt(level: LogLevel, args: Arguments) {
        let mut buf: [u8; 4096] = [0; 4096];
        let len = fmt_to_buf(&mut buf, args);
        unsafe {
            __android_log_write(level_to_prio(level), b"Myfuck\0".as_ptr(), buf.as_ptr());
            myfuck_log_write(level_to_prio(level), buf.as_ptr(), len as i32);
        }
    }
    fn myfuck_write(level: LogLevel, msg: &[u8]) {
        unsafe {
            __android_log_write(level_to_prio(level), b"Myfuck\0".as_ptr(), msg.as_ptr());
            myfuck_log_write(level_to_prio(level), msg.as_ptr(), msg.len() as i32);
        }
    }
