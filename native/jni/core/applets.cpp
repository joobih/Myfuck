...
static main_fun applet_main[] = { su_client_main, resetprop_main, myfuckhide_main, nullptr };
...
int main(int argc, char *argv[]) {
    ...
    string_view base = basename(argv[0]);
    if (base == "myfuck" || base == "myfuck32" || base == "myfuck64") {
        if (argc > 1 && argv[1][0] != '-') {
            // Calling applet via myfuck [applet] args
            --argc;
            ++argv;
        } else {
            return myfuck_main(argc, argv);
        }
    }
