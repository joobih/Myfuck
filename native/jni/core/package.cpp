...
#define ENFORCE_SIGNATURE (!MYFUCK_DEBUG)
...
void preserve_stub_apk() {
    ...
    string stub_path = MYFUCKTMP + "/stub.apk";
...
int get_manager(int user_id, string *pkg, bool install) {
    ...
    auto check_dyn = [&](int u) -> bool {
        ...
        bool mismatch = default_cert && read_certificate(dyn, MYFUCK_VER_CODE) != *default_cert;
        close(dyn);
        if (mismatch) {
    ...
    if (skip_mgr_check.test_and_set()) {
        ...
        auto check_pkg = [&](int u) -> bool {
            ...
            if (stat(app_path, &st) == 0) {
                ...
                int fd = xopen(apk.data(), O_RDONLY | O_CLOEXEC);
                string cert = read_certificate(fd, MYFUCK_VER_CODE);
                close(fd);
