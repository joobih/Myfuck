...
class myfuck_node : public node_entry {
public:
    explicit myfuck_node(const char *name) : node_entry(name, DT_REG, this) {}
    ...
    void mount() override {
        ...
        if (name() == "myfuck") {
            for (int i = 0; applet_names[i]; ++i) {
                ...
                VLOGD("create", "./myfuck", dest.data());
                xsymlink("./myfuck", dest.data());
            }
        } else {
            string dest = dir_name + "/supolicy";
            VLOGD("create", "./myfuckpolicy", dest.data());
            xsymlink("./myfuckpolicy", dest.data());
        }
        create_and_mount(MYFUCKTMP + "/" + name());
    }
};

static void inject_myfuck_bins(root_node *system) {
    ...
    // Insert binaries
    bin->insert(new myfuck_node("myfuck"));
    bin->insert(new myfuck_node("myfuckpolicy"));
...
if (access("/system/bin/app_process" #bit, F_OK) == 0) {                                \
    ...
    string dbin = zygisk_bin + "/myfuck" #bit;                                          \
    string mbin = MYFUCKTMP + "/myfuck" #bit;                                           \
...
void magic_mount() {
    ...
    if (zygisk_enabled) {
        string zygisk_bin = MYFUCKTMP + "/" ZYGISKBIN;
