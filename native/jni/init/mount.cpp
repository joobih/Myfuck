...
void MyfuckInit::mount_rules_dir(const char *dev_base, const char *mnt_base) {
    char path[128];
    ...
    if (xaccess(path, F_OK) == 0) {
        ...
        custom_rules_dir = path + "/myfuck"s;
    } else {
...
void MyfuckInit::setup_tmp(const char *path) {
    LOGD("Setup Myfuck tmp at %s\n", path);
    ...
    close(fd);
    fd = xopen("myfuckinit", O_WRONLY | O_CREAT, 0755);
    xwrite(fd, self.buf, self.sz);
    close(fd);

    // The myfuck binary will be handled later

    // Create applet symlinks
    for (int i = 0; applet_names[i]; ++i)
        xsymlink("./myfuck", applet_names[i]);
    xsymlink("./myfuckinit", "myfuckpolicy");
    xsymlink("./myfuckinit", "supolicy");
