...
static void patch_init_rc(const char *src, const char *dest, const char *tmp_dir) {
    ...
    LOGD("Inject myfuck services: [%s] [%s]\n", pfd_svc, ls_svc);
    fprintf(rc, MYFUCK_RC, tmp_dir, pfd_svc, ls_svc);
...
void SARBase::backup_files() {
    ...
    if (access("/.backup/.myfuck", R_OK) == 0)
        myfuck_cfg = mmap_data("/.backup/.myfuck");
    else if (access("/data/.backup/.myfuck", R_OK) == 0)
        myfuck_cfg = mmap_data("/data/.backup/.myfuck");
}
...
static void extract_files(bool sbin) {
    const char *m32 = sbin ? "/sbin/myfuck32.xz" : "myfuck32.xz";
    const char *m64 = sbin ? "/sbin/myfuck64.xz" : "myfuck64.xz";

    auto myfuck = mmap_data(m32);
    unlink(m32);
...
#define PRE_TMPDIR "/myfuck-tmp"

void MyfuckInit::patch_rw_root() {
...
int myfuck_proxy_main(int argc, char *argv[]) {
    setup_klog();
    LOGD("%s\n", __FUNCTION__);
