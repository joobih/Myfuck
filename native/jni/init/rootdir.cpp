...
#include <myfuck.hpp>
#include <myfuckpolicy.hpp>
#include <utils.hpp>
#include <socket.hpp>

#include "init.hpp"
#include "myfuckrc.inc"
...
bool MyfuckInit::patch_sepolicy(const char *file) {
...
void SARBase::patch_rootdir() {
    ...
    // Extract myfuck
    {
        auto myfuck = mmap_data::ro("myfuck32.xz");
        unlink("myfuck32.xz");
        int fd = xopen("myfuck32", O_WRONLY | O_CREAT, 0755);
        unxz(fd, myfuck.buf, myfuck.sz);
        close(fd);
        patch_socket_name("myfuck32");
        if (access("myfuck64.xz", F_OK) == 0) {
            myfuck = mmap_data::ro("myfuck64.xz");
            unlink("myfuck64.xz");
            fd = xopen("myfuck64", O_WRONLY | O_CREAT, 0755);
            unxz(fd, myfuck.buf, myfuck.sz);
            close(fd);
            patch_socket_name("myfuck64");
            xsymlink("./myfuck64", "myfuck");
        } else {
            xsymlink("./myfuck32", "myfuck");
        }
    }
...
void RootFSInit::patch_rootfs() {
    ...
    // Dump myfuckinit as myfuck
    int fd = xopen("/sbin/myfuck", O_WRONLY | O_CREAT, 0755);
    write(fd, self.buf, self.sz);
    close(fd);
}

void MyfuckProxy::start() {
    // Mount rootfs as rw to do post-init rootfs patches
    xmount(nullptr, "/", nullptr, MS_REMOUNT, nullptr);

    // Backup stuffs before removing them
    self = mmap_data::ro("/sbin/myfuck");
    config = mmap_data::ro("/.backup/.myfuck");
    auto myfuck = mmap_data::ro("/sbin/myfuck32.xz");
    auto myfuck64 = mmap_data::ro("/sbin/myfuck64.xz");
    char custom_rules_dir[64];
    custom_rules_dir[0] = '\0';
    xreadlink(TMP_RULESDIR, custom_rules_dir, sizeof(custom_rules_dir));

    unlink("/sbin/myfuck");
    unlink("/sbin/myfuck32.xz");
    unlink("/sbin/myfuck64.xz");
    rm_rf("/.backup");

    setup_tmp("/sbin");

    // Extract myfuck
    int fd = xopen("/sbin/myfuck32", O_WRONLY | O_CREAT, 0755);
    unxz(fd, myfuck.buf, myfuck.sz);
    close(fd);
    patch_socket_name("/sbin/myfuck32");
    if (myfuck64.sz) {
        fd = xopen("/sbin/myfuck64", O_WRONLY | O_CREAT, 0755);
        unxz(fd, myfuck64.buf, myfuck64.sz);
        close(fd);
        patch_socket_name("/sbin/myfuck64");
        xsymlink("./myfuck64", "/sbin/myfuck");
    } else {
        xsymlink("./myfuck32", "/sbin/myfuck");
    }
    ...
    // Tell myfuckd to remount rootfs
    setenv("REMOUNT_ROOT", "1", 1);
    execv("/sbin/myfuck", argv);
}
