...
constexpr int (*init_applet_main[])(int, char *[]) =
        { myfuckpolicy_main, myfuckpolicy_main, nullptr };
...
static int myfuck_proxy_main(int argc, char *argv[]) {
    setup_klog();
    auto init = make_unique<MyfuckProxy>(argv);
    init->start();
    return 1;
}
...
int main(int argc, char *argv[]) {
    ...
    if (name == "myfuck"sv)
        return myfuck_proxy_main(argc, argv);
