...
// special hacks in myfuckinit to work properly. We do not
...
class SARBase : public MyfuckInit {
protected:
...
class RootFSInit : public MyfuckInit {
private:
    void prepare();
public:
    RootFSInit(char *argv[], BootConfig *config) : MyfuckInit(argv, config) {
        LOGD("%s\n", __FUNCTION__);
    }
    void start() override {
        prepare();
