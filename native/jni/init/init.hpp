...
public:
    MyfuckInit(char *argv[], cmdline *cmd) : BaseInit(argv, cmd) {}
};
...
public:
    SARBase(char *argv[], cmdline *cmd) : MyfuckInit(argv, cmd) {}
};
...
private:
    ...
    void patch_rootfs();
public:
    RootFSInit(char *argv[], cmdline *cmd) : MyfuckInit(argv, cmd) {
        LOGD("%s\n", __FUNCTION__);
    }
    void start() override {
        early_mount();
...
class MyfuckProxy : public MyfuckInit {
public:
    explicit MyfuckProxy(char *argv[]) : MyfuckInit(argv, nullptr) {
        LOGD("%s\n", __FUNCTION__);
    }
    void start() override;
};
