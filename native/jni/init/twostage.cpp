...
#define REDIR_PATH "/data/myfuckinit"

void FirstStageInit::prepare() {
    xmkdirs("/data", 0755);
    xmount("tmpfs", "/data", "tmpfs", 0, "mode=755");
    cp_afc("/init" /* myfuckinit */, REDIR_PATH);

    restore_ramdisk_init();

    {
        auto init = mmap_data("/init", true);
        // Redirect original init to myfuckinit
...
void LegacySARInit::first_stage_prep() {
    ...
    // Replace redirect init with myfuckinit
    ...
    int cfg = xopen("/data/.backup/.myfuck", O_WRONLY | O_CREAT, 0);
    xwrite(cfg, myfuck_cfg.buf, myfuck_cfg.sz);
    close(cfg);
}
