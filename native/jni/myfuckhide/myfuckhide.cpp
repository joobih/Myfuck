...
#include "myfuckhide.hpp"

using namespace std;

[[noreturn]] static void usage(char *arg0) {
    fprintf(stderr,
        "MyfuckHide - Hide Config CLI\n\n"
...
void myfuckhide_handler(int client, ucred *cred) {
...
int myfuckhide_main(int argc, char *argv[]) {
    ...
    case HIDE_NOT_ENABLED:
        fprintf(stderr, "MyfuckHide is not enabled\n");
        goto return_code;
    case HIDE_IS_ENABLED:
        fprintf(stderr, "MyfuckHide is enabled\n");
        goto return_code;
...
void remote_request_hide() {
    ...
    // During process stop, myfuckd will cleanup our mount ns
    read_int(fd);
