...
static void second_stage_entry() {
    ...
    MYFUCKTMP = getenv(MYFUCKTMP_ENV);
#if defined(__LP64__)
...
extern "C" void zygisk_log_write(int prio, const char *msg, int len) {
    // If we don't have log pipe set, ask myfuckd for it
...
static inline bool should_load_modules(uint32_t flags) {
    return (flags & UNMOUNT_MASK) != UNMOUNT_MASK &&
           (flags & PROCESS_IS_MYFUCK_APP) != PROCESS_IS_MYFUCK_APP;
}
...
// The following code runs in myfuckd
...
static vector<int> get_module_fds(bool is_64_bit) {
    ...
    // invalid fd as it will always be /dev/null in myfuckd
    if (is_64_bit) {
#if defined(__LP64__)
...
static void connect_companion(int client, bool is_64_bit) {
    ...
    if (zygiskd_socket < 0) {
        ...
        if (fork_dont_care() == 0) {
            string exe = MYFUCKTMP + "/myfuck" + (is_64_bit ? "64" : "32");
...
static void setup_files(int client, const sock_cred *cred) {
    ...
    if (is_64_bit) {
        hbin = HIJACK_BIN64;
        mbin = MYFUCKTMP + "/" ZYGISKBIN "/myfuck64";
        app_fd = app_process_64;
    } else {
        hbin = HIJACK_BIN32;
        mbin = MYFUCKTMP + "/" ZYGISKBIN "/myfuck32";
    ...
    write_string(client, MYFUCKTMP);
}

static void myfuckd_passthrough(int client) {
