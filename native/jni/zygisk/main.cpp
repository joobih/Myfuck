...
int app_process_main(int argc, char *argv[]) {
    ...
    if (!zygote) {
        ...
        // We have to connect myfuckd via exec-ing myfuck due to SELinux restrictions
...
#if defined(__LP64__)
            execlp("myfuck", "zygisk", "passthrough", buf, "1", (char *) nullptr);
#else
            execlp("myfuck", "zygisk", "passthrough", buf, "0", (char *) nullptr);
#endif
            exit(-1);
        }
        ...
        close(fds[1]);
        if (read_int(fds[0]) != 0) {
            fprintf(stderr, "Failed to connect myfuck daemon, "
    ...
    if (int socket = zygisk_request(ZygiskRequest::SETUP); socket >= 0) {
        do {
            ...
            setenv(MYFUCKTMP_ENV, tmp.data(), 1);
...
static void zygiskd(int socket) {
    ...
    for (;;) {
        ...
        if (pfd.revents && !(pfd.revents & POLLIN)) {
            // Something bad happened in myfuckd, terminate zygiskd
            exit(0);
        }
...
int zygisk_main(int argc, char *argv[]) {
    ...
    if (argc == 3 && argv[1] == "companion"sv) {
        ...
        if (fcntl(client, F_GETFD) < 0)
            return 1;
        if (int myfuckd = connect_daemon(MainRequest::ZYGISK_PASSTHROUGH); myfuckd >= 0) {
            write_int(myfuckd, ZygiskRequest::PASSTHROUGH);
            write_int(myfuckd, is_64_bit);

            if (read_int(myfuckd) != 0) {
                write_int(client, 1);
                return 0;
            }

            write_int(client, 0);
            int real_app_fd = recv_fd(myfuckd);
