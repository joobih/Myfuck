...
check_data() {
  ...
  if grep ' /data ' /proc/mounts | grep -vq 'tmpfs'; then
    ...
    $DATA_DE && [ -d /data/adb/myfuck ] || mkdir /data/adb/myfuck || DATA_DE=false
  fi
  NVBASE=/data
...
run_migrations() {
  ...
  for gz in /data/stock_boot*.gz; do
    ...
    mv $gz /data/myfuck_backup_${LOCSHA1}/boot.img.gz
  done
...
copy_sepolicy_rules() {
  ...
  rm -rf /data/unencrypted/myfuck /cache/myfuck /metadata/myfuck /persist/myfuck /mnt/vendor/persist/myfuck
