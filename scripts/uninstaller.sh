...
is_mounted /data || mount /data || abort "! Unable to mount /data, please uninstall with the Myfuck app"
mount_partitions
check_data
$DATA_DE || abort "! Cannot access /data, please uninstall with the Myfuck app"
get_flags
find_boot_image
...
case $((STATUS & 3)) in
  0 )  # Stock boot
    ...
    ;;
  1 )  # Myfuck patched
    ui_print "- Myfuck patched image detected"
    # Find SHA1 of stock boot image
    SHA1=$(./myfuckboot cpio ramdisk.cpio sha1 2>/dev/null)
    BACKUPDIR=/data/myfuck_backup_$SHA1
    if [ -d $BACKUPDIR ]; then
      ...
      if ! ./myfuckboot cpio ramdisk.cpio "exists init"; then
...
ui_print "- Removing Myfuck files"
rm -rf \
/cache/*myfuck* /cache/unblock /data/*myfuck* /data/cache/*myfuck* /data/property/*myfuck* \
/data/Myfuck.apk /data/busybox /data/custom_ramdisk_patch.sh /data/adb/*myfuck* \
/data/adb/post-fs-data.d /data/adb/service.d /data/adb/modules* \
/data/unencrypted/myfuck /metadata/myfuck /persist/myfuck /mnt/vendor/persist/myfuck
...
if $BOOTMODE; then
  ui_print "********************************************"
  ui_print " The Myfuck app will uninstall itself, and"
  ui_print " the device will reboot after a few seconds"
  ui_print "********************************************"
  (sleep 8; /system/bin/reboot)&
else
  ui_print "********************************************"
  ui_print " The Myfuck app will not be uninstalled"
