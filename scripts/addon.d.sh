...
########################################################
#
# Myfuck Survival Script for ROMs with addon.d support
...
trampoline() {
  ...
  if [ -f $MYFUCKBIN/addon.d.sh ]; then
    ...
    ui_print "***********************"
    ui_print " Myfuck addon.d failed"
    ui_print "***********************"
    ui_print "! Cannot find Myfuck binaries - was data wiped or not decrypted?"
    ui_print "! Reflash OTA from decrypted recovery or reflash Myfuck"
  fi
  exit 1
}
